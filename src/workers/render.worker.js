import { Output, CanvasSource, MediaStreamAudioTrackSource, WebMOutputFormat, BufferTarget } from 'mediabunny';
...
self.onmessage = async (e) => {
    ...
    if (type === 'INIT') {
        ...
        sortedInteractions = interactionsData ? [...interactionsData].sort((a,b) => a.timestamp - b.timestamp) : [];
    ...
    if (type === 'RENDER_FRAME') {
        ...
        // --- DRAWING ---
        const activeOverride = overrides.find(o => timestamp >= o.start && timestamp < o.end);
        ...
        if (backgroundColor.startsWith('url') && backgroundImageBitmap) {
             ...
             if (isPattern) {
                 ...
                 const scale = (settings.backgroundScale * scaleFactor) / backgroundImageBitmap.width;
                 matrix.scaleSelf(scale, scale);
                 pattern.setTransform(matrix);
                 ctx.fillStyle = pattern;
                 ctx.fillRect(0, 0, width, height);
             } else {
                 const imgRatio = backgroundImageBitmap.width / backgroundImageBitmap.height;
        ...
        if (screenBitmap) {
            ...
            if ((settings.showInteractions ?? true) && sortedInteractions.length > 0) {
                ...
                if (mouseEvent) {
                     ...
                     ctx.fillStyle = mouseEvent.type === 'click' ? '#ef4444' : 'rgba(239, 68, 68, 0.5)';
                     ctx.fill();
                }
            }
            ctx.restore();
        ...
        if ((settings.showInteractions ?? true) && sortedInteractions.length > 0) {
             const current = timestamp;
             const windowEvents = sortedInteractions.filter(e => (e.type === 'keydown' || e.type === 'keypress') && e.timestamp <= current && e.timestamp > current - 3.0);
             if (windowEvents.length > 0) {
                  ...
                  const displayableGroups = groups.map(group => {
                      let text = '';
                      const specialKeys = {'Backspace': '⌫', 'Enter': '⏎', 'ArrowUp': '↑', 'ArrowDown': '↓', 'ArrowLeft': '←', 'ArrowRight': '→', 'Escape': '⎋', 'Tab': '⇥', 'Shift': '⇧', 'Control': '⌃', 'Alt': '⌥', 'Meta': '⌘', 'CapsLock': '⇪'};
                      group.forEach(e => {
        ...
        if (settings.showCamera && cameraBitmap && animState.camOpacity > 0.01) {
             ...
             if (camVidRatio > camTargetRatio) {
                 cDrawH = camH; cDrawW = camH * camVidRatio; cDrawY = camY; cDrawX = camX - (cDrawW - camW) / 2;
             } else {
                 cDrawW = camW; cDrawH = camW / camVidRatio; cDrawX = camX; cDrawY = camY - (cDrawH - camH) / 2;
             }
        ...
        const frameDuration = 1/60; // 60 FPS typically
    ...
    // ENCODE_AUDIO intentionally omitted for now (focus on video migration)
