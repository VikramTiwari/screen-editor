...
export const getRandomBackground = () => {
  const types = ['solid', 'gradient', 'pattern'];
  const type = types[Math.floor(Math.random() * types.length)];
  
  if (type === 'solid') {
    return {
      backgroundColor: SOLIDS[Math.floor(Math.random() * SOLIDS.length)],
      backgroundScale: 100
    };
  }
  ...
  if (type === 'pattern') {
    const pattern = SVG_BACKGROUNDS[Math.floor(Math.random() * SVG_BACKGROUNDS.length)];
    const color = SOLIDS[Math.floor(Math.random() * SOLIDS.length)];
    const scale = Math.floor(Math.random() * (500 - 10 + 1)) + 10;
    
    // Replace color in pattern
    const cleanColor = color.replace('#', '');
    const coloredPattern = pattern.replace(/fill='%23[0-9A-Fa-f]{6}'/, `fill='%23${cleanColor}'`);
    
    return {
      backgroundColor: coloredPattern,
